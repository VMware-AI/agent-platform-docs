...
顶栏右侧：搜索、**主题切换**（浅色 / 深色）、**语言切换**（中文 / English）、用户菜单（个人资料 / 关于 / 退出）。

::: tip 侧栏可以折叠
左侧导航支持折叠成纯图标（「折叠侧栏 / 展开侧栏」），屏幕小或者要截图时很有用。折叠状态下鼠标悬停会显示名称。
:::
...
| 项 | 说明 |
|---|---|
| 会话载体 | **httpOnly cookie** —— JS 读不到，也不会存在 localStorage 里 |
| 有效期 | 由 `SESSION_TTL_SECONDS` 控制，**默认 8 小时** |
| 「记住我」 | 勾选后会话跨浏览器重启保留；不勾选则关掉浏览器即失效 |
| 过期表现 | 请求开始返回未认证，页面会跳回登录页 |
| 主动退出 | 用户菜单 → 退出 |

::: tip 会话时长可调
安全要求高的环境可以把 `SESSION_TTL_SECONDS` 调短（例如 3600 = 1 小时），改完 `docker compose -p agent-platform restart backend`。见[环境变量](/reference/env)。
:::
